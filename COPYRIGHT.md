...
> 仓库里出现的曲名、角色、美术、音频、谱面与数据表，**权利全部归原权利人**，
> 本项目**不主张任何权利**；这些内容**仅限本机个人游玩与学习**，
> **禁止二次分发、公开发布与任何商业使用**。本项目按「现状」提供，不附带任何担保，
> **使用后果由使用者自行承担**。若有权利人认为本项目的内容侵犯了其权益，
> **联系即删**（见第七节）。
...
> 本文讲的是**风险和处置**，两份配合看。
>
> 写于 2026-09-12，**2026-09-18、2026-09-24 各复审计过一次**（第三节已更新为 09-24
> 实测数字；09-19 那次「素材瘦身」把一堆文件删了，所以 09-18 版的数字偏大）
> 。这不是法律意见书，但每一条都给出了理由。
...
| 仓库现在干净吗 | **不干净**：**324** 个官方素材/截图文件（`assets/` 255 + `Drafts/` 64 + `docs/` 5）加 **4** 张官方数据表被 git 跟踪（**2026-09-24 实测**，见第三节） |
...
这是本项目**全部的真实风险**所在。逐项审计（**2026-09-24 实测**，`git ls-files`；
内容细分见 **[CREDITS.md](CREDITS.md)** 第四~七节）：

| 内容 | 版权归属 | 是否已被 git 跟踪 | 说明 |
|---|---|---|---|
| `assets/mmw/**`（**214** 个文件：HUD `overlay` + `overlay_opt` 206、notes / 长条 / 触摸线 / stage / effect 顶层 6、ui 关闭与跳过键 2） | SEGA / Colorful Palette | **是（在库！）** | 早期先 commit 后加 gitignore，规则管不了已跟踪文件。**2026-09-19 瘦身已删掉 `effects/`（75）、`sound/`（11）、`font/`（3）、`overlay/ap.mp4`、`overlay/ap-native/**`**，源码里已无引用 |
| `Drafts/**`（**64** 个：官方头像 `profile_icon_*` 42、官方活动图、clear/FC 指示灯草稿） | SEGA / Colorful Palette | **是（在库！）** | 素材暂存区，一直被跟踪；**不在发布包里，但不是 gitignore 的保护范围** |
| `assets/select/**`（**15**：选曲界面全套，含 09-22 加的 `singer.png` / `singerclose.png` 与 `store.png` / `guess.png` / `level.png` / `songlevel.png`） | SEGA / Colorful Palette | **是** | 与 2026-09-12 的结论不同——当天它还是「否」，后来进了库 |
| `assets/se/**`（**21**：UI 音效 + 判定音全套）、`assets/ost/**`（**1**：结算 BGM） | SEGA / Colorful Palette | **是** | 同上，09-12 当天标的是「否」 |
| `assets/fx/**`（**3**：打击特效）、`assets/splashscreen.png`（1） | SEGA / Colorful Palette | **是** | 同上 |
| `musics.json`（401 KB）、`music-vocals.json`（244 KB）、`music-levels.json`（17 KB）、`music-aliases.json`（185 KB） | 官方数据（曲库 / 演唱版本 / 定数表）＋ 社区别名 | **是** | 前三个是事实数据（标题、数字），著作权风险低，但属于官方数据库的整表复制；别名表是社区提交、风险最低 |
| ~~`assets/mmw/font/**`~~（FOT-Rodin ×2 + Noto） | Fontworks / Google | **否（2026-09-19 已删）** | 三个字体文件都已从仓库移除，`--pjsk-font` 一并去掉；现在只读系统字体。详见下方 |
| `docs/**`（**5** 张：3 张 `preview*.png` + `playonseewo.jpg` + `preview_downloader.png`） | 截图里含官方 UI 贴图 | 是 | 游戏截图的著作权风险普遍被视作低（合理使用倾向），但严格说含官方美术 |
| `charts/**`（含 `test.sus`）、`toolchain/` | 谱面数据 / 工具链 | 否 | gitignore 生效 |
...
- ✅ **全仓库的 md 顶部 + 底部都叠了免责声明**（2026-09-24：`AGENTS.md` / `CHARTS.md` /
  `CLI.md` / `CODE-REVIEW.md` / `COPYRIGHT.md` / `CREDITS.md` / `SETUP.md`；`README.md`
  本来就有独立的一节）。第八节那段可直接贴 Release 说明里的文本仍然有效。
...
7. **四张 JSON 表**：保留没问题（事实数据 + 可由 `update_music_db.py` / `setup.sh` 从公开
   接口重建），但别再往里加更多官方表的拷贝。
8. ~~**声明措辞**：补一句「如有侵权请联系移除」~~ —— **2026-09-24 已做**：全仓库 md 都加了
   「非官方 / 素材权利归原权利人 / 仅限本地个人使用 / 无担保 / 责任自负」五条，
   并写明**联系即删**。Release 说明用第八节那段。
...
```
CppSekai 是开源（AGPL-3.0-only）的 Project SEKAI 风格谱面演奏器，由个人业余开发，
与 SEGA / Colorful Palette / Craft Egg 及其关联公司**没有任何隶属、赞助、授权或
认可关系**，亦未获得任何官方认可。遇到问题请勿联系官方客服。

本发布包（exe / 源码 zip）**不含任何官方游戏素材**。谱面、音频、曲绘、UI 贴图
由使用者自行获取，仅供已拥有相应游戏数据的玩家本地研究谱面使用，请于 24 小时内
自行删除，不要分发。素材版权归 SEGA / Colorful Palette 及各词曲版权方所有。

本项目按「现状」提供，不附带任何担保（包括适销性与特定用途适用性）。
使用本软件产生的任何后果由使用者自行承担。
若权利人认为本项目侵犯了其权益，请联系作者，我们会在收到通知后立即移除。
```
...
上面的正文是**给作者看的操作手册**；这一段是**给任何打开这个仓库的人看的**。
两层都读一遍不算多。

1. **非官方**：CppSekai 是个人自制的第三方工具，与 SEGA / Colorful Palette 及
   Project SEKAI 官方**没有任何关系**；官方从未授权、赞助、认可或审核过本项目。
2. **素材权利**：仓库中出现的曲名、角色、美术（`assets/**`、`Drafts/**`、`docs/**`）、
   音频、官方数据表，**著作权与商标权全部归 SEGA / Colorful Palette 及各词曲版权方所有**。
   本项目**不主张任何权利**，也不代表取得了许可。
3. **仅限本地个人使用**：这些素材**只允许在你自己的机器上玩**。
   **不要**提交到公开仓库、**不要**打包分发、**不要**公开传播、**不要**用于商业用途。
   要发布，只发源码或**剥掉全部官方素材**的裸二进制。
...
5. **责任限制**：因下载、编译、运行、修改、分发本项目而产生的**任何直接或间接后果
   ...
   作者不承担任何责任。
6. **联系即删**：若你是权利人并认为本项目侵犯了你的权益，联系作者即可，
   我们会在收到通知后立即移除相关内容，无需先行投诉。

---

*最后强调一次：以上是风险评估和社区实践总结，**不是法律意见**。这里每一条判断都可能
被推翻，涉及真实纠纷请找真律师。来源台账见 [CREDITS.md](CREDITS.md)。*
