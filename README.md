...
| 演奏                      | 选歌                             |
| ------------------------- | -------------------------------- |
| ![演奏](docs/preview.png) | ![选歌](docs/preview_group_reading.png) |
...
- **设置齐全**（H 键或选曲界面的设置按钮，分演奏 / 画面 / 判定 / 系统 / 账户 / 关于六页）：音频偏移、音符速度、BGM / 音效音量、分辨率预设、窗口模式、帧率上限、播放进度条开关、AUTOPLAY 预览、判定窗口、严格 Flick 方向、Flick 视作 Tap、失焦自动暂停、SMTC 汇报开关、昵称 / 学校 / 签名，全部持久化到 `userdata.json`。
...
**Q：为什么没有歌 / 列表里是空的？**
A：仓库不自带谱面与音频（官方素材不入库）。三条路：`bash setup.sh --charts` 下两张测试谱、双击 `build/chartdl.exe` 批量下载、或者自己把文件丢进 `charts/`。命名必须是 `<4位id>_<难度>.sus`，其余规则、下载地址、URL 模板见 **[CHARTS.md](CHARTS.md)**。

**Q：放进去了却不显示 / 歌名显示成 `0075 master`？**
A：前者基本是文件名不符合 `<4位id>_<难度>.sus`（或没按 F5 重扫）；后者是缺 sidecar 的 `title`。CHARTS.md 的「常见问题」表里逐条对过。

**Q：完全没有声音 / 音乐比谱面晚约 9 秒？**
A：没声音 = 谱面目录里少了对应的 mp3（`<id4>.mp3` 或 `se_/vs_/an_<id4>_<n>.mp3`）。晚 9 秒 = 官服 mp3 开头的静音填充没识别出来，sidecar 里写 `"fillerSec": 9.0` 即可（正常情况下程序会自己扫）。

**Q：判定手感不对，总觉得偏？**
A：先调 **设置 → 演奏 → 音频偏移**（±2000 ms，实时生效），对着判定线找这个设备的延迟；再调 **设置 → 判定** 的判定窗口 / 长条容错。启动日志里有一行 `[settings] windows perfect=… great=… good=… bad=… missAfter=… holdTail=… linked=…`，写的正是当前生效的那套数，汇报手感问题带上它最省事。

**Q：长条（hold）什么时候算断？**
A：尾判是**松手判定**——结束前在容错窗口内松手给 Perfect / Great / Good，一直按到底也是 Perfect，只有明显提前松手才断连。窗口在 **设置 → 判定 → 长条容错**（松手 / 起按两个数）。

**Q：键盘打不出左右 flick？**
A：键盘一个键没有方向，只能发向上 flick；左右方向必须用鼠标或触摸。触摸屏上滑很难触发的话，开 **设置 → 判定 → Flick 视作 Tap**（含方向，任意点按都能清）。

**Q：手柄能打歌吗？**
A：不能，手柄只做菜单（选曲 / 设置 / 弹窗走方向键 + A/B）。12 轨的东西手柄打不了，这是有意的。

**Q：触摸屏能用吗？多指呢？**
A：能。触摸走 SDL 的 touch→mouse 合成，多指对应多轨；不想看到系统那圈触摸涟漪可以在设置里关掉。

**Q：怎么和别人一起打（同机联机）？**
A：**设置 → 系统 → 多人游玩**（会顺带允许多开）：每个窗口一个玩家，第一个进来的是房主，负责选曲和 BGM，其他人各自挑难度、一起开始。走同机共享内存，不需要网络，也没有服务器。想临时开一次用 `--party`。

**Q：为什么这首歌没有「切换歌手」/ 面板里只有一个版本？**
A：面板只列出**你谱面目录里真的存在音频的演唱版本**（`se_/vs_/an_/cl_<id>_<n>.mp3`），有几个就显示几张卡。切版本只换音轨，谱面是共用的，不会复制 SUS。

**Q：界面字太小 / 太大？**
A：**设置 → 画面 → 界面缩放**。只作用于选曲和结算；演奏界面故意不跟着缩放（那种界面是拿来打的，不是拿来读的）。

**Q：AUTOPLAY 会不会写成绩？**
A：不会。AUTOPLAY（和命令行的 `--auto`）都不记分、不涨等级。
...
**Q：需要先装什么？**
A：什么都不用。`bash setup.sh` 会把 zig 0.14.1 工具链和贴图 / 音效一起拉下来，然后 `bash build.sh`。不需要 Visual Studio、cmake、vcpkg、Python。

**Q：Windows 7 能跑吗？**
A：能（SP1 起），但要自己装一次 **UCRT / VC++ 2015-2022 运行库**；Win10+ 开箱即用。媒体浮层（SMTC）与任务栏进度条只有 Win10+ 才有，Win7 上这两项自动没有。
...
**Q：改渲染 / 改 UI 怎么不走肉眼看？**
A：`--screenshot` 能不开窗口跑一帧存 PNG，`--auto` 能跑完整局并断言判定统计；无头自检、退出码、各种调试开关见 **[CLI.md](CLI.md)**。

### 数据与许可

**Q：成绩和设置存在哪？换电脑怎么带走？**
A：`userdata.json`（和 `charts/` 同级）。多用户时是 `profiles/<id>.json` 加一份 `profiles/index.json` 记当前档案。换机器把 `charts/` 和 `userdata.json`（或整个 `profiles/`）一起拷走就行——成绩按谱面**文件名**存，所以两边文件名一致才认得出。

**Q：想清空进度 / 换成另一个人玩？**
A：删掉 `userdata.json` 就是一个全新账户；多用户直接新建档案，各自独立的设置 / 成绩 / 等级，互不干扰。

**Q：会联网吗？会上传什么吗？**
A：游戏本体**没有任何网络代码**，断网照跑。只有 `chartdl.exe` 下载谱面时会联网（地址来自公开的资源镜像），也不上传任何东西。

**Q：能改代码 / 二次分发吗？**
A：可以——代码是 **AGPL-3.0-only**：改了要开源，发二进制要附许可并指向源码。素材（`assets/`、`charts/`）属于 SEGA / Colorful Palette，只限本地游玩，别随包发。细节见下面的「授权与素材」。
