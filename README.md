...
- **等级（Player Rank）**：照搬原版那套——一局给的经验就是这局的**分数评级倍率**（D 20 / C 200 / B 240 / A 280 / S 320；原版乘算的「加成倍率」这里没有加成系统，所以恒为 1），升级所需经验用官方曲线（1 级 10、2 级 8010，3~12 级每级 +500，13~15 级每级 +1000，16 级起每级 +480，上限 900 级）。选曲界面右上角一块「等级」牌，结算画面面板右下角一块（带经验条和 `+N EXP`，升级时写「等级提升!」）。
