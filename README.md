...
- **XBOX 手柄**（插上就能用）可以操作菜单：选曲、设置、弹窗都走方向键 + A/B，但**不参与打歌**。
...
- **选歌 → 演奏 → 结算 → 暂停弹窗 → 3-2-1 倒计时 → 继续演出**，一套完整流程：`确定` / `重试` / `放弃` / `继续演出`，通关与 FULL COMBO 连同设置一起存进 `userdata.json`。**血量掉到 0 的曲子不算通关**（打完但没活着 = failed，只在选曲界面留最高分，不打 CLEAR）。开场卡片右上角有圆形的跳过键（白底藏青 `>>`，pjsk 的按键样式），直接跳到谱面 0 秒开打。
...
- **等级（Player Rank）**：照搬原版那套——一局给的经验就是这局的**分数评级倍率**（D 20 / C 200 / B 240 / A 280 / S 320；原版乘算的「加成倍率」这里没有加成系统，所以恒为 1），升级所需经验用官方曲线（1 级 10、2 级 8010，3~12 级每级 +500，13~15 级每级 +1000，16 级起每级 +480，上限 900 级）。选曲界面右上角一块「等级」牌（和排序 / 分组下拉框一样高、一样平；**左端那截绿色是离下一级还有多少经验**），结算画面面板右下角同一块，下面写 `+N EXP`，升级时写「等级提升!」。
- **账户是纯本地的**：昵称、学校 / 组织、个性签名只写在 `userdata.json` 里，**平时一句都不显示**——只有设置「账户」页，以及点选曲界面右上角那块等级牌弹出的**个人资料卡**会画出来。
- **界面音效**：UI 点击、弹窗开 / 关、选曲每滑一格、难度切换各有官方音效；一次操作只出一声（按下就弹窗的那种，弹窗音会把点击音顶掉，不会双击）。
...
| 手柄 | `A` / `START` / `Y` | 开打（= `Enter`）/ 设置面板（= `H`，演奏中则是暂停）/ 重新扫描（= `F5`） |
...
| 演奏 | 音符速度 | 1–12，实时生效 |
| 演奏 | BGM 音量 | 0–100%，实时生效（谱面音轨 / 选曲试听 / 结算 BGM 一起变） |
| 演奏 | 音效音量 | 0–100%，实时生效（判定音 + 界面点击音） |
...
| 判定 | Flick 视作 Tap | flick 音符按 tap 判——任意点按/滑动都能清掉（含方向），给上滑很难触发的触摸屏用。长条尾部的 flick 不转换：那个音符直接消失，按住不放就过 |
...
| 文件 | 位置 | 作用 |
|---|---|---|
| `userdata.json` | 谱面目录那一层（`build/` 布局下就是仓库根，和 `charts/` 并排） | 玩家数据：`settings`（全部设置）+ `scores`（通关 / FULL COMBO / 最高分）+ `account`（昵称 / 学校 / 签名 / 等级 / 经验 / 游玩次数）。成绩按谱面**文件名**作 key，所以换机器重新下载同样的谱，把这份文件拷到 `charts/` 旁边成绩就还在；老文件（没有 `account` 段）直接按默认账户读，不会报错 |
