...
在班上垃圾希沃使用 Windows 原生触摸享受和同学打烤的快乐。
...
- **SUS 谱面原生解析 + 演奏**：12 轨，所有音符全部支持。
- **最牛逼的性能优化**：没有 Electron，没有引擎，release 带了资源都只有 6.9MB，纯 C++ 编写性能负担极低，老赛扬都能玩。
- **自带音乐商店**：国服日服随便下。
- **判定、结算页面对齐原作**：一比一高度还原。
- **三种输入共用一条路径**：键盘 12 键、鼠标左/右键（当成两个指针）、多指触摸。**XBOX 手柄只做菜单**，不参与打歌。
- **特效、时钟、对轴都不糊弄**：命中特效走核心自带的 pjsk 粒子系统，官服 mp3 开头 ~9 秒静音自动扫掉，不用手动对轴。
- **本地多人**：同一台机器开几个窗口一起打，和同学在班上拼机。
- **别名搜索数据库**：自带超大数据库，海底谭你搜海底捞，モニタニング你搜视奸都能识别。
...
| 手柄 | `B` / `BACK` | 关掉当前弹窗或设置卡，等同 `ESC` 的「返回」 |
...
**Q：判定手感不对，总觉得偏？**
A：先调 **设置 → 演奏 → 音频偏移**；再调 **设置 → 判定** 的判定预设 / 判定窗口 / 长条容错。
