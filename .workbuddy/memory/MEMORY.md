...
## 数据文件（`userdata.json`）
- 位置：有 `<exe>\..\charts` 就用那一层（build/ 布局 = 仓库根），否则 `<exe>\userdata.json`。
- 内容 `{settings, scores}`；scores 的 key 是**谱面文件名**，每条 `{cleared, fullCombo,
  bestScore}`（bestScore 是 2026-09-13 加的，给结算画面的最高得分/新纪录用）。
- 优先级：命令行 > userdata.json > 内置默认；`--screenshot` 模式**不写**这个文件。
